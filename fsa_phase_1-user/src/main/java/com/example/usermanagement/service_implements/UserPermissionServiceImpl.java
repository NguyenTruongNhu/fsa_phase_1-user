...
//import com.example.usermanagement.repositories.UserDAO;
...
//    private final UserDAO userDAO;
    ...
    public ResponseEntity<ResponseObject> grantPermission(int userID, Role role) {
        var person = userRepository.findById(userID).orElse(null);
        var permission = userPermissionRepository.findUserPermissionByRole(role).orElse(null);
        if (person == null) {
            return null;
        }
        ...
        return ResponseEntity.ok(new ResponseObject("Successful", "Change user permission", userRepository.save(person)));
    }
